# How to use Dockerized ASK AFADI Library

Use the Dockerized version of ASK AFADI Library for a much faster and complete startup of ASK AFADI Library.

### Minimum Requirements

> [!TIP]
> Running ASK AFADI Library on AWS/GCP/Azure?
...
\*ASK AFADI Library by default uses a built-in vector database powered by [LanceDB](https://github.com/lancedb/lancedb)

\*ASK AFADI Library by default embeds text on instance privately [Learn More](../server/storage/models/README.md)

## Recommend way to run dockerized ASK AFADI Library!
...
> the docker container using ASK AFADI Library as `localhost:xxxx` will not resolve for the host system.
>
...
> when used in ASK AFADI Library.
...
<table>
<tr>
<th colspan="2">Mount the storage locally and run ASK AFADI Library in Docker</th>
</tr>
<tr>
<td>
  Linux/MacOs
...
Go to `http://localhost:3001` and you are now using ASK AFADI Library! All your data and progress will persist between
...
The integrations below are templates or tooling built by the community to make running the docker experience of ASK AFADI Library easier.

### Use the Midori AI Subsystem to Manage ASK AFADI Library

Follow the setup found on [Midori AI Subsystem Site](https://io.midori-ai.xyz/subsystem/manager/) for your host OS
After setting that up install the ASK AFADI Library docker backend to the Midori AI Subsystem.
...
Then in docker you need to replace that localhost part with `host.docker.internal`. For example, if running Ollama on the host machine, bound to http://127.0.0.1:11434 you should put `http://host.docker.internal:11434` into the connection URL in ASK AFADI Library.
